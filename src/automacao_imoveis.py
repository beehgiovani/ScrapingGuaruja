...
async def handle_captcha_flow(page, next_selector):
    ...
    for attempt in range(max_attempts):
        # Safety: Não ficar mais que 5 minutos no mesmo captcha
        if time.time() - start_time > 300:
            print("🛑 Timeout Global (5 min) no fluxo de Captcha.")
            return False
...
def atomic_write_json(data, filename):
    ...
    # OTIMIZADO: Reduzido de 5 para 3 retries para velocidade
    max_retries = 3
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                time.sleep(0.05)  # OTIMIZADO: Reduzido de 0.1 para 0.05s
                continue
            else:
...
async def run():
    ...
    if use_tor:
        print("=== Tor IP Masking Enabled (Stream Isolation) ===")
        
        if not tor.start():
            print("⚠ WARNING: Failed to start Tor. Proceeding without IP masking.")
            use_tor = False
        else:
            # Stream Isolation: cada worker usa credenciais únicas na MESMA porta
            # Tor automaticamente dá IPs diferentes para credenciais diferentes
            assigned_socks_port = 9050
            assigned_control_port = 9051
            
            # Credenciais únicas por worker para isolamento de stream
            tor_username = f"worker{shard_idx}"
            tor_password = f"session{shard_idx}"
            
            print(f"🌐 Worker {shard_idx} usando Stream Isolation")
            print(f"   Porta: {assigned_socks_port} | Credentials: {tor_username}:***")
            
            # Check IP via the assigned port with credentials
            try:
                proxies = {
                    'http': f'socks5://{tor_username}:{tor_password}@127.0.0.1:{assigned_socks_port}',
                    'https': f'socks5://{tor_username}:{tor_password}@127.0.0.1:{assigned_socks_port}'
                }
                ...
                print(f"✓ Worker {shard_idx} Tor IP: {real_ip} (isolated)")
                print(f"✓ IP successfully masked with stream isolation")
            except Exception as e:
    ...
    # OTIMIZADO: Fast-fail se arquivo grande demais (> 50MB)
    if os.path.exists(legacy_file):
        try:
            file_size_mb = os.path.getsize(legacy_file) / (1024 * 1024)
            if file_size_mb > 50:
                print(f"⚠️ Legacy file muito grande ({file_size_mb:.1f}MB). Pulando carregamento para velocidade.")
            else:
    ...
    async with async_playwright() as p:
        ...
        async def rotate_browser_session():
            ...
            if use_tor:
                try:
                    socks_port = assigned_socks_port
                    
                    # Stream isolation: usar credenciais únicas por worker
                    launch_options['firefox_user_prefs'] = {
                        ...
                        # Credenciais para isolamento de stream (IP único por worker)
                        "network.proxy.socks_username": tor_username,
                        "network.proxy.socks_password": tor_password
                    }
                except Exception as e:
        ...
        async def process_inscription(inscricao_target, display_name, active_page):
            ...
            try:
                # 2. Coletar Área
                print(f"  > [{inscricao_target}] Navigating to Area Page...")
                await geo_page.goto("https://scimpmgsp.geometrus.com.br/mctm_lancamentos/index_certidao_valor_venal", timeout=45000)
                
                print(f"  > [{inscricao_target}] Handling Area Captcha...")
                passou = await handle_captcha_flow(geo_page, '#MctmLancamentoIdentificacao')
                if not passou: raise Exception("Captcha Area Falhou")
                
                print(f"  > [{inscricao_target}] Searching for Inscription...")
                await geo_page.fill('#MctmLancamentoIdentificacao', inscricao_target)
                await geo_page.click('input[type="submit"]')
                # OTIMIZADO: Timeout de 30s para não travar no networkidle
                try:
                    await geo_page.wait_for_load_state("networkidle", timeout=30000)
                except:
                    print(f"  > [{inscricao_target}] Networkidle timeout (30s), proceeding anyway...")
                ...
                if link_relative:
                    ...
                    print(f"  > [{inscricao_target}] Opening Certidão Popup...")
                    await geo_page.goto(popup_url, timeout=45000)
                    try:
                        await geo_page.wait_for_load_state("networkidle", timeout=30000)
                    except:
                        pass # Proceed with extraction even if networkidle fails
                ...
                if not skip_boleto_search:
                    print(f"  > [{inscricao_target}] Navigating to Boleto List...")
                    await geo_page.goto("https://scimpmgsp.geometrus.com.br/mvia2_boletos/lista", timeout=45000)
                
                    print(f"  > [{inscricao_target}] Handling Boleto Captcha...")
                    ...
                    for link_relative in potential_links:
                        try:
                            ...
                            await geo_page.goto(popup_url_boleto, timeout=90000)
                            try:
                                await geo_page.wait_for_load_state("networkidle", timeout=90000)
                            except:
                                pass
                ...
                # Detect connection errors or blocks
                is_connection_error = any(keyword in error_str for keyword in ['timeout', 'connection', 'network', 'refused', 'nav error', 'exceeded'])
                is_block_detected = any(keyword in error_str for keyword in ['bloqueio', 'block', 'captcha', 'acesso negado', 'forbidden'])
                
                if is_connection_error:
                    result["status_processamento"] = "erro_conexao"
                    print(f"🔴 ERRO DE CONEXÃO/TIMEOUT detectado em {inscricao_target}")
                elif is_block_detected:
                    result["status_processamento"] = "erro_bloqueio"
                    print(f"🛡️ BLOQUEIO detectado em {inscricao_target}")
                else:
                    result["status_processamento"] = f"erro: {str(e)}"
                    print(f"❌ ERRO GENÉRICO detectado em {inscricao_target}: {e}")
        ...
        for i, lote_obj in enumerate(lotes_para_processar):
            ...
            # IP Rotation is handled ONLY on errors (not time-based)
            ...
            while True:
                for attempt in range(max_retries):
                    if attempt > 0:
                        # Check if previous attempt had connection/block error
                        # AGRESSIVO: Se falhou com qualquer erro em Tor, tentamos trocar de IP
                        should_rotate = False
                        if final_result and final_result.get("status_processamento"):
                            status = final_result.get("status_processamento", "").lower()
                            # Rotacionar em qualquer erro de rede, bloqueio ou erro genérico se estiver usando Tor
                            if any(x in status for x in ["erro_conexao", "erro_bloqueio", "erro:"]):
                                should_rotate = True
                                print(f"🔄 Gatilho de Rotação Ativado: {status}")
                        
                        if should_rotate:
                            print(f"⚠️ Retry {attempt+1}/{max_retries} for {inscricao} (IP Rotation)")
                            await rotate_browser_session()  # Rotate ONLY on connection/block errors
                        else:
                            print(f"⚠️ Retry {attempt+1}/{max_retries} for {inscricao} (sem rotação)")
                    ...
                    if "erro" in res["status_processamento"].lower():
                        # If error, save result for next iteration to check if rotation needed
                        final_result = res
                        continue
                    else:
                        final_result = res
            ...
            if should_scan_subs:
                ...
                while True:
                    ...
                    # IP Rotation handled ONLY on errors (removed time-based rotation)

                    # OTIMIZADO: Reduzido de 0.5s para 0.3s
            ...
            # Sleep between lots
            # OTIMIZADO: Reduzido de 1s para 0.5s para velocidade
